...
void significance_effSplot_BDT_ele_0_31() 
{

  /* Set the weighted number of signal and background events for the ele sample */
  Double_t nS = 14.2 ;
  Double_t nB = 85886.56 ;

  /********************* Make the canvas *************************/
  TCanvas* c = new TCanvas("c","c",900,675) ;
  c->SetTitle("Significance vs. Signal efficiency, BDT method, electron sample") ;
  c->Update() ;
  auto legend = new TLegend(0.5,0.7,0.9,0.9) ;
  legend->SetHeader("Significance vs. Signal efficiency, BDT, electron") ; 
 
  /****** Get plot of effB vs effS from ROOT file as graph *******/
  TFile* file31 = TFile::Open("/eos/user/j/jdioquin/www/MVA_studies/MultiMethodPlot/NewNtuples31718/BDT_training_ele.root") ;
  ...
  g_sig31->SetTitle("Significance vs. Signal efficiency, BDT method, electron sample;effS;Significance") ;
  
  gPad->SetGrid() ;
  //TF1 fit("fit","pol6") ;
  //g_sig31->Fit(&fit) ;
  
  TFile* file0 = TFile::Open("/eos/user/j/jdioquin/www/MVA_studies/MultiMethodPlot/OriginalNtuples/BDT_training_ele.root") ;
  ...
  g_sig0->SetTitle("Significance vs. Signal efficiency, BDT method, electron sample;effS;Significance") ;
  ...
  legend->Draw() ;
  c->Update() ;
  c->Print("Plots/significance_effS_BDT_ele_0_31.pdf") ;
}
