...
void isolation_pipt_2dhists() {
  ...
  TH2D* isolation_pipt_sig_mu = new TH2D("isolation_pipt_sig_mu","#Sigmap_{T} and p_{T}^{#pi} signal, muon channel;p_{T}^{#pi}(GeV);#Sigmap_{T}",50,20.,150.,50,0.,12) ;
  TH2D* isolation_pipt_bkg_mu = new TH2D("isolation_pipt_bkg_mu","#Sigmap_{T} and p_{T}^{#pi} background, muon channel;p_{T}^{#pi}(GeV);#Sigmap_{T}",50,20.,150.,50,0.,14) ;
  TH2D* isolation_pipt_sig_ele = new TH2D("isolation_pipt_sig_ele","#Sigmap_{T} and p_{T}^{#pi} signal, electron channel;p_{T}^{#pi}(GeV);#Sigmap_{T}",50,20.,150.,50,0.,12) ;
  TH2D* isolation_pipt_bkg_ele = new TH2D("isolation_pipt_bkg_ele","#Sigmap_{T} and p_{T}^{#pi} background, electron channel;p_{T}^{#pi}(GeV);#Sigmap_{T}",50,20.,150.,50,0.,14) ;
  ...
  for(i=0;i<N;i++) {
    tsig_mu->GetEntry(i) ;
    isolation_pipt_sig_mu->Fill(pi_pT_sig_mu,piRelIso_05_ch_sig_mu*pi_pT_sig_mu,weight_sig_mu) ;
  }
  N=tbkg_mu->GetEntries() ;
  for(i=0;i<N;i++) {
    tbkg_mu->GetEntry(i) ;
    isolation_pipt_bkg_mu->Fill(pi_pT_bkg_mu,piRelIso_05_ch_bkg_mu*pi_pT_bkg_mu,weight_bkg_mu) ;
  }
  N=tsig_ele->GetEntries() ;
  for(i=0;i<N;i++) {
    tsig_ele->GetEntry(i) ;
    isolation_pipt_sig_ele->Fill(pi_pT_sig_ele,piRelIso_05_ch_sig_ele*pi_pT_sig_ele,weight_sig_ele) ;
  }
  N=tbkg_ele->GetEntries() ;
  for(i=0;i<N;i++) {
    tbkg_ele->GetEntry(i) ;
    isolation_pipt_bkg_ele->Fill(pi_pT_bkg_ele,piRelIso_05_ch_bkg_ele*pi_pT_bkg_ele,weight_bkg_ele) ;
  }
  ...
  c1->Update() ;
  c1->Print("Isolation_pipt_pdfs/piIso_pT_2dhists.pdf(","pdf") ;
  ...
  c2->Update() ;
  c2->Print("Isolation_pipt_pdfs/piIso_pT_2dhists.pdf)","pdf") ;
} 
